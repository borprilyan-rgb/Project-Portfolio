...
def show_area_calculator():
    ...
    with tab1:
        ...
        # --- TOP SETUP CONTROLS ---
        with st.expander("Floor Settings", expanded=False):
        ...
        with col_btn1:
            if st.button("➕ Add floor (Top)", width='stretch'):
...
class ConstructionProject:
    def __init__(self, data, pt_data=None):
        """
        data: st.session_state.projects[curr_id]["data"]
        pt_data: The default template data for the building type
        """
        # 1. Measurement Data (Unique to this Project)
        self.land_area = data.get("m_land", 0.0)
        self.gba = data.get("m_gba", 0.0)
        self.gfa = data.get("m_gfa", 0.0)
        self.sgfa = data.get("m_sgfa", 0.0)
        
        # 2. Architecture & Interior
        self.rooms = data.get("m_rooms", 0.0)
        self.lobby = data.get("m_lobby", 0.0)
        self.carpet = data.get("m_carpet", 0.0)
        self.glass = data.get("m_glass", 0.0)
        
        # 3. Sanitary (Uses Template Defaults if missing)
        self.san_qty_room = data.get("r_san_qty", pt_data.get("san_room_qty", 0) if pt_data else 0)
        self.toilet_m = data.get("m_toil_m", 0.0)
        self.toilet_f = data.get("m_toil_f", 0.0)
        self.toilet_d = data.get("m_toil_d", 0.0)
        self.mushola = data.get("m_mushola", 0.0)

        # 4. Facilities
        self.fac_res = data.get("m_fac_res", 0.0)
        self.fac_pub = data.get("m_fac_pub", 0.0)
        self.land_m2 = data.get("m_land_m2", 0.0)
        self.misc_switch = data.get("misc_switch", 0)

    @property
    def total_toilets(self):
        """Example of how logic stays in the class, not the UI"""
        return self.toilet_m + self.toilet_f + self.toilet_d
...
with c1:
    st.button("Tambah", on_click=cb_add_project, type="primary", width='stretch')
    
with c2:
    unlock_delete = st.sidebar.toggle("Use Password to Delete")
    ...
    if unlock_delete:
        ...
        if pw_input == MASTER_DELETE_PW:
            ...
            if st.button("Hapus", disabled=not can_delete, type="secondary", width='stretch'):
                del st.session_state.projects[st.session_state.current_proj_id]
                st.session_state.current_proj_id = list(st.session_state.projects.keys())[0]
                save_data()
                st.rerun()
            
            if st.sidebar.button("⚠️ HAPUS SEMUA DATA", type="primary", width='stretch'):
                st.session_state.projects = {"proj_1": {"name": "New Project 1", "type": "Hotel", "data": {}}}
                st.session_state.proj_counter = 1
                st.session_state.current_proj_id = "proj_1"
                save_data()
